...
#' @param info is the factor of class label
...
#' @export
#' 
sda_ranking <- function(prototype=NULL,info=NULL,fdr=FALSE,ranking.score="avg",lambda=0.5){
  library(sda)

  if(is.null(prototype) || is.null(info)){
    stop("You must insert Prototypes and Class labels!\n")
  }else{
    sda.ranking(Xtrain=t(prototype),fdr=FALSE,ranking.score="avg",L=info,lambda=0.5) -> prototype.sda.rank
