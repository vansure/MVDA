...
#' @param cartella is the path where the function will save the statistics files
#' @keywords multi-view clustering; feature-relevance; correlation
#' @export
printStatistics<-function(DB, clust_vector,center,cartella){
    library("gdata")
    myApply <- lapply
    DB <- as.matrix(DB)
    ...
    write.table(toPrint,paste(cartella,"correlazioni_centroidi_cluster.txt",sep="\t"))
    pdf(file=paste(cartella,"/MeanCorrelation.pdf",sep=""))
    ...
    #dev.new()
    #X11()
    pdf(file=paste(cartella,"/AverageCorrelation.pdf",sep=""))
    ...
    #dev.new()
    #x11()
    pdf(file=paste(cartella,"/SDCorrelation.pdf",sep=""))
