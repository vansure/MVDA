...
#' @param cl.cla is the vector of labels for each cluster
#' @param info is the factor of patient classes
#' @return a matrix containing a pvalue for each cluster for each class label.
#' @export
#' 
fisher_test <- function(clust,cl.cl,CM_sup, info){ 
  require(MASS)
  
  nCluster <- dim(CM_sup)[1]
  nClass <- length(names(table(info)))
  ...
  cl.cl <- names(table(info))[cl.cl]
  fisherTest <- cbind(fisherTest,cl.cl)
  colnames(fisherTest) <- c(names(table(info)),"Cluster class")
